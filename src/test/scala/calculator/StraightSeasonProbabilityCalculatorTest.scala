...
import model.{AytoFixtures, Pairing, ComleteProbabilityForWoman, Scenario, StraightSeason}
...
class StraightSeasonProbabilityCalculatorTest extends FlatSpec with Matchers with AytoFixtures {
  "StraightSeasonProbabilityCalculator" should "calculate probabilities from existing confirmed matches and no matches" in {
    ...
    val expectedProbabilities = Set(
      ComleteProbabilityForWoman("a", Map("d" -> 1.00, "e" -> 0.00, "f" -> 0.00)),
      ComleteProbabilityForWoman("b", Map("d" -> 0.00, "e" -> 1.00, "f" -> 0.00)),
      ComleteProbabilityForWoman("c", Map("d" -> 0.00, "e" -> 0.00, "f" -> 1.00)))
  ...
  it should "calculate probabilities when there is a mix of confirmed information and non confirmed information" in {
    ...
    val expectedProbabilities = Set(
      ComleteProbabilityForWoman("a", Map("d" -> 0.5, "e" -> 0.00, "f" -> 0.5)),
      ComleteProbabilityForWoman("b", Map("d" -> 0.25, "e" -> 0.5, "f" -> 0.25)),
      ComleteProbabilityForWoman("c", Map("d" -> 0.25, "e" -> 0.5, "f" -> 0.25)))
